...
2. **Pagination violations** — ✅ Shipped Phase 3 (PRs `813b0d5`, `2ba6f6a`, `fe62eb2`). The original 4 sites (ShipmentsPage, OrdersPage, BulkOrdersPage, shipmentsService default) all paginate at `limit=20` now via the new `Pagination` primitive in `src/components/ui/`. Audit during PR 4 surfaced **two more** `limit=100` defaults still in place: [teamService.getTeam](../src/services/teamService.ts) and [clientsService.getClients](../src/services/clientsService.ts) — both intentional pending TeamPage / ClientsPage pagination + customer-picker autocomplete. See `docs/IMPLEMENTATION_PLAN.md` § Phase 3 follow-ups.
