...
export type DetailTab = 'overview' | 'warehouse' | 'records' | 'payment' | 'images' | 'timeline';

export type Mode = 'air' | 'sea' | 'd2d' | '';

// ── Invoice (1:1 with order, may not exist yet) ─────────────────────────────
...
function readInvoice(record: AnyRecord | null): OrderInvoice | null {
  const raw = asRecord(pick(record, ['invoice']));
  if (!raw) return null;
  const id = readString(raw, ['id']);
  if (!id) return null;
  return {
    id,
    invoiceNumber: readString(raw, ['invoiceNumber']),
    status: (readString(raw, ['status']) || 'draft') as InvoiceStatus,
  };
}
...
export function toView(order: ApiOrder): OrderView {
  ...
  return {
    ...
    shippingMark: readString(record, ['shippingMark']) || null,
